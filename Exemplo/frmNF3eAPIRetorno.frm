VERSION 5.00
Begin VB.Form frmNF3eAPIRetorno 
   ...
   Begin VB.ComboBox comboTpDown 
      Height          =   315
      ItemData        =   "frmNF3eAPIRetorno.frx":0000
      Left            =   120
      List            =   "frmNF3eAPIRetorno.frx":0013
      TabIndex        =   26
      Text            =   "X"
      Top             =   8880
      Width           =   2055
   ...
   End
End
Attribute VB_Name = "frmNF3eAPIRetorno"
...
On Error GoTo SAI
    ...
    'lendo nProt(Protocolo de Autorizcao) retornado no JSON recebido da API
    protocolo = LerDadosJSON(result, "nProt", "", "")
    If txtStatusSefaz <> "100" Then
        txtnProt.Text = "Nao Possui"
    Else
...
On Error GoTo SAI
    ...
    'lendo o responsetext, que e onde esta ou estarao os xml's, pdf's, JSON's conforme o tipo informado
    result = downloadNF3eESalvar(txtToken.Text, txtChaveRetorno.Text, comboTpDown.Text, txtTpAmb.Text, "C:\Documentos", isShow)
    'result = downloadNF3e(txtToken.Text, txtChaveRetorno.Text, comboTpDown.Text)
    txtResult.Text = result
    
    Exit Sub
SAI:
    txtResult.Text = ("Problemas ao Requisitar emiss�o ao servidor" & vbNewLine & Err.Description)
End Sub
